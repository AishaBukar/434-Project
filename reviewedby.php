<?php
   ...
   /* require_once('config.php');
    ...
    $query = "SELECT e.firstname,e.lastname, s.mark_abstract,s.mark_literature_review,s.mark_methodology,s.mark_analysis,s.mark_conclusion 
                FROM examiner e JOIN studentandexaminer ON 
                e.Id = s.lecturerID
                WHERE project_name LIKE('')";//work on dynamically calling the project name that was cliceked on review page by the admin 
    ...
    <body>
        <table>
            ...
            if($no_rows>0){
                ...
                echo "Project has not been graded by any examiner ";
            }
            ?>
        </table><br>
